// /api/_lib/send-order-email.js
// Lógica compartilhada de montagem e envio do email de pedido via Resend.
// Usada tanto por /api/notify-order.js (disparo pelo navegador do cliente)
// quanto por /api/webhook-mp.js (disparo pelo servidor do Mercado Pago,
// rede de segurança caso o cliente feche o navegador antes de voltar).

export async function sendOrderEmail({ cart, customer, dlv, addr, selDate, frete, total, source }) {
        ...
        html: `
          <div style="font-family:sans-serif;max-width:600px;margin:0 auto;background:#0A0908;color:#F5F0E8;padding:32px;border-radius:8px">
            ...
            <div style="background:#C41E24;padding:16px;border-radius:6px;text-align:center">
              ...
              <p style="margin:4px 0 0;font-size:12px;opacity:.8">Pagamento confirmado via Mercado Pago${source ? ' · ' + source : ''}</p>
            </div>
